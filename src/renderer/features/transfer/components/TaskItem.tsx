...
export const TaskItem: React.FC<TaskItemProps> = ({ task }) => {
  ...
  const { sessions, connections } = useSessionStore()

  const session = sessions.find(s => s.sessionId === task.sessionId)
  const connection = session
    ? connections.find(c => c.connectionUuid === session.connectionUuid)
    : undefined
  const sessionName = connection?.name ?? connection?.host ?? task.sessionId
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            ...
            title={`${connection?.protocol?.toUpperCase()}://${connection?.host}:${connection?.port}`}
          >
            {connection?.protocol?.toUpperCase()} {sessionName}
          </div>
          {task.status === 'active' && (
            <div className="mt-2">
              <div className="flex items-center gap-2 mb-1">
